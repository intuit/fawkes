...
def split_data(articles, labels):
    ...
    return (train_articles, train_labels, validation_articles,
            validation_labels)
...
def train(articles, labels):
    ...
    validation_sequences = article_tokenizer.texts_to_sequences(
        validation_articles)
...
def train_lstm_model():
    ...
    for app in app_configs:
        ...
        if not (CATEGORIZATION_ALGORITHM in app_config
        ...
        dump_json(article_tokenizer.to_json(),
                  LSTM_ARTICLE_TOKENIZER_FILE.format(app_name=app_config[APP]))

        # Saving the tokenizers
        dump_json(label_tokenizer.to_json(),
                  LSTM_LABEL_TOKENIZER_FILE.format(app_name=app_config[APP]))


def predict_labels(articles, app_config, model, article_tokenizer,
                   label_tokenizer):
    ...
    # Add padding
    tokenized_articles = pad_sequences(tokenized_articles,
                                       maxlen=MAX_LENGTH,
                                       padding=PADDING_TYPE,
                                       truncating=TRUNC_TYPE)
