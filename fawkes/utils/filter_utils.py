...
def filter_disabled_review_channels(app_config):
    return [
        ...
        if review_channel.is_channel_enabled == True
    ]
...
def filter_reviews_by_channel(reviews, review_channels):
    return [review for review in reviews if review.channel_name in review_channels]


# Given a set of reviews and a earliest date-time, filters our reviews which are before the time
def filter_reviews_by_time(
    reviews, earliest_date_time, latest_date_time=datetime.now(timezone.utc)
):
    return [
        review
        for review in reviews
        if review.timestamp > earliest_date_time and review.timestamp < latest_date_time
    ]
