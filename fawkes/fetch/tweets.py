...
def fetch(review_channel):
    api = twitter_authenthication(
        review_channel.consumer_key,
    ...
    for twitter_handle in review_channel.twitter_handle_list:
        ...
        query_results = search_handle_mentions(
            api, twitter_handle, datetime.today().strftime("%Y-%m-%d")
        )
        ...
        while len(query_results[constants.KEY_CONTAINING_TWEETS]) > 0:
            ...
            latest_id = (
                query_results[constants.KEY_CONTAINING_TWEETS][-1][
                    constants.KEY_WITH_STATUS_ID
                ]
                - 1
            )
            query_results = search_handle_mentions(
                api, twitter_handle, datetime.today().strftime("%Y-%m-%d"), latest_id
            )
