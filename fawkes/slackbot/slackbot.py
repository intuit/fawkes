...
def get_jira_details(review, app_config, issue_type):
    ...
    description = (
        review.message
        + "\n\n Details : \n {code:json}"
        + json.dumps(review.to_dict(), indent=4)
        + "{code}"
    )
...
def get_people_to_tag(app_config, review):
    ...
    if app_config.slack_config.slack_notification_rules.keyword_based_rules != None:
        for (
            keyword
        ) in app_config.slack_config.slack_notification_rules.keyword_based_rules:
            if keyword in review.message:
                list_of_slack_ids += app_config.slack_config.slack_notification_rules.keyword_based_rules[
                    keyword
                ]
...
def send_review_to_slack(
    slack_url, slack_channel, review, app_config, isUnifiedJSON=True
):
    """
    ...
    if not isUnifiedJSON:
        ...
        attachment = {
            ...
            "color": get_sentiment_color(review.derived_insight.sentiment["compound"]),
            "pretext": "Sentiment : "
            + str(review.derived_insight.sentiment["compound"]),
            "author_name": review.derived_insight.category,
            "author_link": "",
            "author_icon": "https://i.imgur.com/pydBDO7.png",
            "text": review.message + constants.SPACE + user_mention_string,
            "ts": time.mktime(review.timestamp.timetuple()),
            "footer": review.app_name
            + " : "
            + review.channel_name
            + " | User Id: "
            + str(review.user_id),
            "actions": get_actions(review, app_config),
        }
        ...
        if review.channel_type == ReviewChannelTypes.TWITTER:
            ...
            response = requests.post(
                slack_url, data=json.dumps(payload), headers=headers
            )
        ...
        # App Store and Play Store
            ...
            attachment["pretext"] = generate_star_from_rating(review.rating)
...
def send_reviews_to_slack(fawkes_config_file=constants.FAWKES_CONFIG_FILE):
    ...
    for app_config_file in fawkes_config.apps:
        ...
        logging.info(
            logs.OPERATION, FawkesActions.PUSH_SLACK, "ALL", app_config.app.name
        )
        ...
        for review in reviews:
            send_review_to_slack(
                app_config.slack_config.slack_hook_url,
                app_config.slack_config.slack_channel,
                review,
                app_config,
            )
